    ...
    return Form(
        ...
        child: SingleChildScrollView(
          ...
          child: Row(
            ...
            children: [
              const Expanded(
                child: ImageCustomUser(action: true, create: true,)
              ),
              Expanded(
            ...
            action: () async{
                if(widget.formProductAddKey.currentState!.validate()){
                  ...
                  final user = User(
                    ...
                    dateCreate: Formatter.dateFormat(), 
                    dateUpdate: Formatter.dateFormat(), 
                    photoPath: '', 
                    photoURL: ''
                  );
                  if(await ref.read(userProvider.notifier).addUser(user, ref.read(photoProvider))){
                    if(context.mounted){
                      ref.read(drawerIndexProvider.notifier).update((state) => state = 5,);
                      displayInfoBar(context, builder: (context, close) {
                        return InfoBar(
                          title: const Text(Strings.successAlert),
                          content: Text('${Strings.successUserContent} ${user.dni} ${Strings.successCreateContent}'),
                          action: IconButton(
                      ...
                      displayInfoBar(context, duration: const Duration(seconds: 5), builder: (context, close){
                        return InfoBar(
                          ...
                          content: Text('${Strings.successUserContent} ${user.dni} no ${Strings.successCreateContent}'),
                          action: IconButton(
                            icon: const Icon(FluentIcons.clear),
                            onPressed: close,
                          ),
